...
func main() {
	...
	validatePath := flag.String("validate", "", "只验证管道结构（不执行），检查 DAG/连接器/依赖")
	...
	// -validate 模式：只验证管道结构（DAG/连接器/依赖），不执行。
	if *validatePath != "" {
		p, err := pipeline.LoadFromFile(*validatePath)
		if err != nil {
		...
		if err != nil {
			fmt.Fprintln(os.Stderr, "❌ 拓扑排序失败:", err)
			os.Exit(1)
		}
		fmt.Printf("✅ 管道 %q 验证通过（%d 步，执行顺序: %s）\n", p.Name, len(p.Steps), joinIDs(order))
		return
	}
