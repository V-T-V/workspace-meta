...
func writeStdout(w io.Writer, rows pipeline.Rows, format string) error {
	...
	case "json-compact":
		return writeJSONCompact(w, rows)
	case "table":
		return writeTable(w, rows)
	case "csv":
		return writeCSV(w, rows)
	default:
		return fmt.Errorf("不支持的 stdout 格式 %q（支持 json/json-pretty/json-compact/table/csv）", format)
	}
}
...
// writeJSONCompact 输出紧凑 JSON（每行一条，无缩进，适合管道/脚本消费）。
func writeJSONCompact(w io.Writer, rows pipeline.Rows) error {
	if len(rows) == 0 {
		_, err := fmt.Fprintln(w, "[]")
		return err
	}
...
func writeTable(w io.Writer, rows pipeline.Rows) error {
	...
	fmt.Fprintln(w, strings.Join(headerParts, sep))
	// 分隔线
