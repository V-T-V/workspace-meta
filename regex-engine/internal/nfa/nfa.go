...
// HasLazy 报告正则中是否出现过非贪婪量词（*? / +? / ??）。
// NFA 结构本身不区分贪婪/非贪婪（Thompson 状态集合模拟并行推进所有状态，
// 最终接受态可达性相同），但 matcher 在多个可达接受位置中"选哪个"由它决定：
// 无非贪婪 → 选最远（最长，贪婪）；有非贪婪 → 选最近（最短，惰性优先）。
func (n *NFA) HasLazy() bool {
	return n.hasLazy
}
