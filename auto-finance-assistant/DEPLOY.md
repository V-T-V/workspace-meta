...
> 版本 1.4.0 · 单 EXE 17.5MB · Windows 原生部署
...
|------|----------|------------------|
| 对话 | `qwen2.5:1.5b` | `qwen3.5:4b` |
| 向量 | `nomic-embed-text` | `nomic-embed-text` |
...
将 `auto-finance-assistant-deploy-v1.4.0.zip` 解压到目标目录，例如：
...
> **不创建 config.yaml 也能启动**：程序会用默认配置（qwen2.5:1.5b / 127.0.0.1:8080）。
...
ollama:
  ...
  chat_model: qwen2.5:1.5b        # 生产改 qwen3.5:4b
...
|----|-----------------|---------------------|
| 模型 | qwen2.5:1.5b (1.9GB) | qwen3.5:4b (2.5GB) |
...
# {"status":"ok","database":"ok","ollama":"ok","model":"qwen2.5:1.5b"}
```
...
**A:** CPU 上 1.5B 模型每次回答 10-30 秒属正常。生产环境用 RTX 4060 + Qwen3.5 4B 可降至 2-5 秒。
