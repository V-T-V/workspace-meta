...
> **9 个 Milestone 全部完成** · M1-M9 全部交付并 CPU 验证 · 生产部署只需换配置（RTX 4060 + Qwen3.5 4B）
...
- **模型**：Ollama 本地模型（生产 Qwen3.5 4B / 开发 qwen2.5:1.5b）
...
| 配置文件 | 环境 | 模型 | 推理 |
|----------|------|------|------|
| `config.example.yaml` | 生产（RTX 4060 8GB） | `qwen3.5:4b` | `num_gpu` GPU offload |
| `config.dev.yaml` | 开发机（AMD 4750U CPU） | `qwen2.5:1.5b` | `num_thread:8` CPU |
...
ollama pull qwen2.5:1.5b       # CPU 验证模型（生产用 qwen3.5:4b）
...
Windows 11
│
├─ Ollama (qwen2.5:1.5b / qwen3.5:4b + nomic-embed-text)
│
