...
用法:
  ...
  workspace-ops info   [-config ...] [-slug xxx]  导出项目详情 JSON
...
// runInfo: 导出单个项目的详情 JSON（含 facts + test_status）。
func runInfo(args []string) error {
	fs := flag.NewFlagSet("info", flag.ExitOnError)
	configPath := fs.String("config", "config.dev.yaml", "配置文件路径")
	slug := fs.String("slug", "", "项目 slug（必填）")
	_ = fs.Parse(args)
	if *slug == "" {
		return fmt.Errorf("用法: ops info -slug <project-name>")
	}
	cfg, _, _ := loadCfg(*configPath)
	dbPath, _ := cfg.ResolveDBPath()
	db, err := storage.Open(dbPath)
	if err != nil {
		return err
	}
	defer func() { _ = db.Close() }()
	pid, err := storage.GetProjectIDBySlug(db, *slug)
	if err != nil {
		return fmt.Errorf("项目 %q 未找到（先运行 scan）", *slug)
	}
	facts, err := storage.ProjectFacts(db, pid, 0)
	if err != nil {
		return err
	}
	// 转 JSON 输出
	...
	if tr, _ := storage.LatestTestRunForProject(db, pid); tr != nil {
		out["test_status"] = tr.Status
		out["test_duration_ms"] = tr.DurationMs
		out["test_command"] = tr.Command
	}
